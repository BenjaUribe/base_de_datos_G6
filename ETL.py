...
def sincronizar_y_mostrar():
    with psycopg2.connect(**TRANSACCIONAL) as conn_trans, psycopg2.connect(**ANALISIS) as conn_ana:
        ...
        for row in cur_trans.fetchall():
            ...
            region_nombre = suc[0] if suc else None  # region (transaccional)
            comuna_nombre = suc[1] if suc else None  # comuna (transaccional)

            # Mapeo: region (transaccional) -> comuna (analítica), comuna (transaccional) -> region (analítica)
            # Buscar el id_region real usando el nombre de la comuna de la transaccional
            id_region = None
            if comuna_nombre:
            ...
            # Buscar el id_comuna real usando el nombre de la región de la transaccional
            id_comuna = None
            if region_nombre:
                cur_ana.execute("SELECT id_comuna FROM comuna WHERE comuna_nombre = %s", (region_nombre,))
                com = cur_ana.fetchone()
                if com:
                    id_comuna = com[0]
                else:
                    # Insertar nueva comuna si no existe
                    cur_ana.execute("INSERT INTO comuna (comuna_nombre) VALUES (%s) RETURNING id_comuna", (region_nombre,))
                    id_comuna = cur_ana.fetchone()[0]
...
if __name__ == "__main__":
    ...
    print("Iniciando ETL en modo background con schedule cada 1 minuto...")
    etl_thread = threading.Thread(target=background_scheduler, daemon=True)
    etl_thread.start()
    try:
        while True:
            time.sleep(60)
